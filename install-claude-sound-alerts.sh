...
PYEOF
  ...
  # The figures Claude Code reports, saved by the status line. These come from
  # Anthropic, so they cover every surface, not just this terminal.
  if [ -f "$LIMITS" ]; then
    local fp fa wp wa
    ...
    [ -n "$fa" ] && ok "5 hour limit: ${fp:-?}% used, resets $(human_at "$fa")"
    [ -n "$wa" ] && ok "7 day limit:  ${wp:-?}% used, resets $(human_at "$wa")"
  else
    warn "no limit figures yet"
    dim "they arrive once Claude Code draws its status line; restart it if you have only just installed"
  fi
...
# A reset time, said the way a person would: the clock time, plus how long that
# is from now, because "18:40" alone still needs mental arithmetic.
human_at() {
  local at="$1" now left when
  case "$at" in ''|*[!0-9]*) printf 'unknown'; return ;; esac
  now=$(date +%s)
  when=$(date -d "@$at" '+%H:%M' 2>/dev/null || date -r "$at" '+%H:%M' 2>/dev/null)
  left=$(( at - now ))
  if [ "$left" -le 0 ]; then
    printf '%s (already passed)' "${when:-?}"
  elif [ "$left" -lt 3600 ]; then
    printf '%s (in %dm)' "${when:-?}" $(( left / 60 ))
  elif [ "$left" -lt 86400 ]; then
    printf '%s (in %dh %dm)' "${when:-?}" $(( left / 3600 )) $(( (left % 3600) / 60 ))
  else
    printf '%s (in %dd)' "${when:-?}" $(( left / 86400 ))
  fi
}
...
pack_count=0
seen_packs=""
while IFS='|' read -r sound_rel sound_b64; do
  [ -n "$sound_rel" ] || continue
  [ -n "$sound_b64" ] || continue
  # Each entry is "pack/name.wav". Refuse anything that tries to climb out.
  case "$sound_rel" in
    */../*|../*|/*) continue ;;
  esac
  sound_pack_name=${sound_rel%%/*}
  mkdir -p "$SOUND_DIR/$sound_pack_name"
  case " $seen_packs " in
    *" $sound_pack_name "*) ;;
    *) seen_packs="$seen_packs $sound_pack_name"; pack_count=$((pack_count + 1)) ;;
  esac
  if printf '%s' "$sound_b64" \
     | "$PY" -c 'import base64,sys; sys.stdout.buffer.write(base64.b64decode(sys.stdin.read()))' \
     > "$SOUND_DIR/$sound_rel" 2>/dev/null; then
    sound_count=$((sound_count + 1))
  else
    rm -f "$SOUND_DIR/$sound_rel"
  fi
done <<'SOUNDSEOF'
...
if [ "$sound_count" -gt 0 ]; then
  ok "wrote $sound_count sounds in $pack_count pack(s)"
else
